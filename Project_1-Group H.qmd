...
```{r}
# Function to plot the mean value of the statistic
library(dplyr)
library(ggplot2)
...
plot.state <- function(df, var_name = "number") {
  ...
  summarize_df <- df |>
    filter(division != "ERROR") |> # Remove observations with "ERROR" in Division
    group_by(division, year) |>  #across the years for each Division
...
plot.county <- function(df, state_of_interest = "AL", var_name = "number", top_or_bottom = "top", num_counties = 5) {
  # Filter the data to only include the specified state
  state_data <- df |>
...
plot.county(combined_processed_data[[1]],state_of_interest = "AL", var_name = "number", top_or_bottom = "top", num_counties = 5)
```
